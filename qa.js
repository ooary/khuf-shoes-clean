...
(async()=>{
 ...
 const layout=await page.evaluate(()=>{const v=document.querySelector('.review-viewport').getBoundingClientRect(),cards=[...document.querySelectorAll('#reviewRail>.google-review-card')],c=cards[0].getBoundingClientRect(),gap=parseFloat(getComputedStyle(document.querySelector('#reviewRail')).gap),left=getComputedStyle(document.querySelector('.review-edge-left')),right=getComputedStyle(document.querySelector('.review-edge-right'));return {viewport:v.width,card:c.width,gap,renderedCards:cards.length,overflow:getComputedStyle(document.querySelector('.review-viewport')).overflow,leftGradient:left.backgroundImage,rightGradient:right.backgroundImage}}); const visible=(layout.viewport+layout.gap)/(layout.card+layout.gap); if(visible<3||visible>3.5||layout.renderedCards!==12||layout.overflow!=='hidden'||!layout.leftGradient.includes('gradient')||!layout.rightGradient.includes('gradient')) throw Error('horizontal marquee layout '+JSON.stringify(layout));
